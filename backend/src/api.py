...
import logging, iaServer, apiMapper, flaskUtil
...
def postMessage():
    (errRes, user, question, history, ability) = flaskUtil.getReqParams(request, ['user', 'question', 'history', 'ability'])
    if isinstance(errRes, Response): return errRes
    return flaskUtil.setResponseOK(apiMapper.markDownToHtml(iaServer.ask(user, question, history, ability)))

# @app.route('/api/v1/chat/<string:user>', methods=['GET'])
@app.get('/api/v1/chat/<string:user>')
def getMessages(user):
    if not user: return flaskUtil.setResponseKO('User is required')
