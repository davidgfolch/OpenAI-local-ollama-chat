...
from service.aiService import (getModels, ERROR_OPENAI_GET_AVAILABLE_MODELS,
...
@pytest.fixture
def mock_openaiUtil():
    with patch('aiService.openaiUtil') as mock:
        yield mock
...
def test_invoke_exception():
    with patch('aiService.with_model') as mock_with_model:
...
def test_getMessages():
    with patch('aiService.get_session_history') as mock_get_session_history:
